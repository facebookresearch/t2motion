        ...
        with Progress(transient=True) as progress:
            ...
            for keyid in dataset.keyids:
                ...
                for index in range(cfg.number_of_samples):
                    ...
                    joints_g = prepare_joints(model.Datastruct(features=batch["connect_datastruct"].features).joints.squeeze(0).numpy())
                    joints_p = prepare_joints(model.Datastruct(features=connect_features).joints.squeeze(0).numpy())
                    jitters.append(calc_jitter(torch.tensor(joints_p)))
                    jittersG.append(calc_jitter(torch.tensor(joints_g)))
                    footskating.append(calc_footskating(joints_p))
                    footskatingG.append(calc_footskating(joints_g))
                    groundpenetrating.append(calc_ground_penetrating(joints_p))
                    groundpenetratingG.append(calc_ground_penetrating(joints_g))
            ...
            for k,v in metrics.items():
                res_dict[k] = v
            res_dict["Jitter-G"] = (sum(jittersG)/len(jittersG)).item()
            res_dict["Jitter-P"] = (sum(jitters)/len(jitters)).item()
            res_dict["FootSkating-G"] = sum(footskatingG)/len(footskatingG)
            res_dict["FootSkating-P"] = sum(footskating)/len(footskating)
            res_dict["GroundPenetrate-G"] = sum(groundpenetratingG)/len(groundpenetratingG)
            res_dict["GroundPenetrate-P"] = sum(groundpenetrating)/len(groundpenetrating)

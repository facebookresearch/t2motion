    ...
    for trim in trims:
        if not len(new_trims):
            ...
            if trim-new_trims[-1]<=length_thresh:
                ...
                inter_index = 1
                while inter_trim<trim and inter_index<num_seqs:
...
new_dict = {}
match_dict = {}
for k,v in amass_path_ann.items():
    new_dict[k] = v
    if v["path"] not in match_dict:
        match_dict[v["path"]] = {k:v["trim"]["start"]}
    else:
        flag=True
        for id, start in match_dict[v["path"]].items():
            if start == v["trim"]["start"]:
                new_dict[k]["match_id"] = id
                new_dict[id]["match_id"] = k
                match_dict[v["path"]].pop(id)
                flag=False
                break
        if flag:
            match_dict[v["path"]][k] = v["trim"]["start"]

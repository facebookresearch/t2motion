...
class SMPLH(Rots2Joints):
    ...
    def forward(self, smpl_data: dict,
        ...
        # matrix_poses shape [nframes,22,3,3]
        ...
        if nohands:
            from ems.tools.easyconvert import to_matrix
            # still axis angle
...
def smplh_to(jointstype, data, trans):
    from ems.info.joints import get_root_idx

    if "mmm" in jointstype:
        from ems.info.joints import smplh2mmm_indexes
        indexes = smplh2mmm_indexes
        data = data[..., indexes, :]

        # make it compatible with mmm
        if jointstype == "mmm":
            from ems.info.joints import smplh_to_mmm_scaling_factor
        ...
        from ems.info.joints import smplh2smplnh_indexes
        ...
        raise NotImplementedError(f"SMPLH to {joinsttype} is not implemented.")

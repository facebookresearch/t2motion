    ...
    def __init__(self, datapath: str,
        ...
        for i, keyid in enumerator:
            ...
            if not self.sampler.accept(self.kitml_correspondances[keyid]["num_frames"]):
                num_bad += 1
                continue
            if self.kitml_correspondances[keyid]["prev"] != -1:
                if self.kitml_correspondances[str(self.kitml_correspondances[keyid]["prev"])]["num_frames"]<2:
                    self.kitml_correspondances[keyid]["prev"] = -1
            if self.kitml_correspondances[keyid]["next"] != -1:
                if self.kitml_correspondances[str(self.kitml_correspondances[keyid]["next"])]["num_frames"]<2:
                    self.kitml_correspondances[keyid]["next"] = -1
            if self.kitml_correspondances[keyid]["match_id"] != -1:
                if self.kitml_correspondances[str(self.kitml_correspondances[keyid]["match_id"])]["num_frames"]<2:
                    self.kitml_correspondances[keyid]["match_id"] = -1   

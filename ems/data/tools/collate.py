...
def collate_datastruct_and_text(lst_elements: List) -> Dict:
    if "mask_length" in lst_elements[0]:
        # need reorg list elements
        # TBD MODIFY MAE
        collate_datastruct = lst_elements[0]["datastruct"][0].transforms.collate
        datastruct_lst = []
        length_lst = []
        mask_datastruct_lst = []
        indices_lst = []
        for lst_element in lst_elements:
            datastruct_lst.extend(lst_element["datastruct"])
            length_lst.extend(lst_element["length"])
            mask_datastruct_lst.extend(lst_element["mask_datastruct"])
            indices_lst.extend(lst_element["indices"])
        batch = {
             "datastruct": collate_datastruct(datastruct_lst),
             "length": length_lst,
             "mask_datastruct": collate_datastruct(mask_datastruct_lst),
             "indices": indices_lst
         }
    else:

...
class DistilbertActorAgnosticEncoder(DistilbertEncoderBase):
    def __init__(self, modelpath: str,
        ...
        self.skel_embedding = nn.Linear(135, latent_dim)
    ...
    def forward(self, texts: List[str], motions: Tensor, lengths: Optional[List[int]] = None) -> Union[Tensor, Distribution]:
        ...
        y = motions
        y = self.skel_embedding(y)
        y = y.permute(1, 0, 2)  # now it is [nframes, bs, latent_dim]
        y_mask = lengths_to_mask(lengths, device)
        #[46,8,256]
        if self.hparams.vae:
            ...
            aug_mask = torch.cat((token_mask, mask, y_mask), 1)
        else:
        ...
        yseq = self.sequence_pos_encoding(y)
        # 10,8,256
        xseq = torch.cat((xseq,yseq),0)

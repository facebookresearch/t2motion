...
def render(npydata, frames_folder, *, mode, faces_path, gt=False,
           ...
           oldrender=True,res="high", init=True, contacts = False):
    if init:
    ...
    if mode == "video":
        ...
        if downsample and not is_mesh:
            print(downsample)
            raise RuntimeError("check data size")
            npydata = npydata[::8]
    elif mode == "sequence":
    ...
    if is_mesh:
        if contacts:
            print("Contact Prediction Render")
            from .contact_meshes import Meshes
            data = Meshes(npydata, gt=gt, mode=mode,
                      faces_path=faces_path,
                      canonicalize=canonicalize,
                      always_on_floor=always_on_floor)
        else:
            print("No Contact Prediction Render")
    ...
    for index, frameidx in enumerate(frameidx):
        if mode == "sequence":
            ...
            if contacts and is_mesh:
                mat = data.mat[frameidx]
            else:
                mat = data.mat
